...
class TestFastPLBF_M(unittest.TestCase):
    ...
    def test_fit(self):
        for fastplbf in TestFastPLBF_M.filters:
            assert fastplbf.is_fitted_

        
    def test_FN(self):
        for fastplbf in TestFastPLBF_M.filters:
            self.assertTrue(sum(fastplbf.predict(TestFastPLBF_M.objects[~TestFastPLBF_M.labels]) == 0))

    def test_M(self):
        nonkeys = TestFastPLBF_M.objects[~TestFastPLBF_M.labels]
